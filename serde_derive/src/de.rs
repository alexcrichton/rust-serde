...
impl Parameters {
    ...
    fn type_name(&self) -> &str {
        self.this.segments.get(self.this.segments.len() - 1).item().ident.as_ref()
    }

    fn de_lifetime_def(&self) -> syn::LifetimeDef {
        syn::LifetimeDef {
            attrs: Vec::new(),
            lifetime: syn::Lifetime::new(Term::intern("'de"), Default::default()),
            bounds: self.borrowed.iter().cloned().collect::<Vec<_>>().into(),
...
impl<'a> ToTokens for DeImplGenerics<'a> {
    fn to_tokens(&self, tokens: &mut Tokens) {
        ...
        for lifetime in mem::replace(&mut generics.lifetimes, Default::default()) {
            if !lifetimes.trailing_delim() {
                lifetimes.push_trailing(Default::default());
            }
...
impl<'a> ToTokens for DeTyGenerics<'a> {
    fn to_tokens(&self, tokens: &mut Tokens) {
        ...
        let lifetime = syn::Lifetime::new(Term::intern("'de"), Default::default());
        let mut lifetimes = Delimited::new();
